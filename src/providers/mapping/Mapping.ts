...
//TODO remove dependency on ionic here?
...
export class Mapping {
    ...
    constructor(events: Events, private logging: Logging) {
        ...
        this.debouncedMapPositionUpdate = IonicUtil.debounce(() => {
    ...
    showPOIOnStaticMap(mapcanvasID: string, poi, includeMapLink: boolean = false, isRunningUnderCordova: boolean = false, mapWidth: number = 200, mapHeight: number = 200) {
        ...
        if (mapCanvas != null) {
            ...
            if (includeMapLink == true) {
                mapHTML += "<div>" + IonicUtil.formatMapLink(poi, "<div><img width=\"" + width + "\" height=\"" + height + "\" src=\"" + mapImageURL + "\" /></div>", isRunningUnderCordova) + "</div>";
            } else {
